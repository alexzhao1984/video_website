...
class Post:
    def new(self, title, bankuai, content, filename, user_id):
        if user_id:
            return db.insert('posts', title=title, bankuai=bankuai, content=content, filename=filename, user_id=user_id)
        else:
            return 0
    ...
    def view(self, id):
        '''获取id对应的文章'''
        posts = db.query('''SELECT posts.id, title, bankuai, content, filename, posts.time, user_id, users.name AS username, users.picture AS user_face, visit
                            FROM posts JOIN users
                            ON posts.user_id = users.id
                            WHERE posts.id = %d''' % id)
        posts2 = db.query('''SELECT posts.id, title, content, filename, posts.time, user_id, users.name AS username, users.picture AS user_face, visit
                            FROM posts JOIN users
                            ON posts.user_id = users.id
                            WHERE posts.id = %d''' % id)
        if posts:
            db.update('posts', where='id=%d'%id, visit=int(posts2[0].visit)+1)
            return posts[0]
        return None
    ...
    def list_together(self,page):
        page_posts1, page_count1=self.list(page)
        page_posts2, page_count2=self.list2(page)

        return ([page_posts1, page_posts2],[page_count1, page_count2])
    ...
    def list2(self, page):
        ...
        posts=call_cmd('cd ./static && ls *.mp4').split('\n')
        page_posts = []
        i=0
        for p in posts:
            if p == '':
                continue
            i+=1
            page_posts.append({'id': p, 'title': p, 'userid': p, 'username': p, 'comment_count': p, 'last_time': p})
    ...
    def list(self, page):
        ...
        # 获取从offset开始共per_page个post
        offset = (page - 1) * per_page
        posts = db.query('''SELECT posts.id, title, bankuai, posts.time, user_id, users.name AS username, visit
                            FROM posts JOIN users
                            ON posts.user_id = users.id
        ...
        for p in posts:
            ...
            page_posts.append({'id': p.id, 'title': p.title, 'bankuai': p.bankuai, 'userid': p.user_id, 'username': p.username, 'comment_count': comment.count(), 'last_time': last_time, 'visit': p['visit']})
