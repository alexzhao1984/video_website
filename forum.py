...
#C:\Users\zhaob>C:\Users\zhaob\Downloads\ffmpeg-20180508-293a6e8-win64-static\ffmpeg-20180508-293a6e8-win64-static\bin\ffmpeg.exe -i "C:\Users\Public\Videos\Sample Videos\Wildlife.wmv" -vcodec h264 -s 320*240 -an -f mp4 "C:\Users\Public\Videos\Sample Videos\Wildlife.mp4"
urls = (
  '/', 'Index2',
  '/add', 'Add',
  '/edit/(\d+)', 'Edit',
...
def render2(params={}, partial=False):
    global_vars = dict(settings.GLOBAL_PARAMS.items() + params.items())
    
    return web.template.render(templates, globals=global_vars)
    
def titled_render2(subtitle=''):
    subtitle = subtitle + ' - ' if subtitle else ''
    return render2({'title': subtitle + settings.SITE_NAME, 'make_html': util.make_html,
                   'trim_utf8': util.trim_utf8, 'menu': util.menu(model.User())})
...
class Index2:
    def GET(self):
        i = web.input(page='1')
        page = int(i.page)
        page_posts, page_count = model.Post().list_together(page)
        return titled_render().list2(page_posts, page_count, page)
    ...
    def POST(self):
        ...
        if i['upload_file'].filename=='':
            ...
            if bn==1 and i['upload_file'].filename.split('.')[-1]!='mkv':
                cmd='ffmpeg -i '+fn+' -vcodec h264 -s 320*240 -f mp4 '+'static/%s'%i['upload_file'].filename+'.mp4'
                '''r=os.system(cmd)
                if r!=0:
                    print '转码失败'
                ...
                fn=u+fn
        post_id = model.Post().new(i.title, i.bankuai, i.content, fn, model.User().current_id())
        if post_id:
